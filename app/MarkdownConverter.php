...
class MarkdownConverter extends BaseConverter
{
	public function __construct(array $config = [])
	{
		...
		$config['embed'] = [
			'adapter' => new class implements EmbedAdapterInterface {
				/** @param Embed[] $embeds */
				public function updateEmbeds(array $embeds): void
				{
					foreach ($embeds as $i => $embed) {
						$url = parse_url($embed->getUrl());
						parse_str($url['query'], $query);
						
						$video = match ($url['host']) {
							'youtube.com' => match ($url['path']) {
								'/playlist' => sprintf('https://www.youtube-nocookie.com/embed/videoseries?showinfo=1&list=%s', $query['list']),
								'/watch' => sprintf('https://www.youtube-nocookie.com/embed/%s', $query['v']),
							},
							'youtu.be' => sprintf('https://www.youtube-nocookie.com/embed/%s', trim($url['path'], '/')),
							default => null,
						};
						...
						if ($video) {
							$embed->setEmbedCode(
								<<<HTML
								<iframe
									class="aspect-video w-full my-4 rounded-lg"
									src="{$video}" 
			...
			'allowed_domains' => ['youtube.com', 'youtu.be'], // 'bsky.app', 'github.com'],
			'fallback' => 'link',
		];
				...
				protected function parseNode(Node $node): array
				{
					...
					if ($child instanceof Text && preg_match('#^\[!(note|tip|important|warning|caution)]$#i', $child->getLiteral(), $matches)) {
						...
						return match ($matches[1]) {
							'note' => [
								'border-l-4 border-blue-700 pl-4 py-2 max-w-4xl',
							...
							'tip' => [
								'border-l-4 border-green-700 pl-4 py-2 max-w-4xl',
							...
							'important' => [
								'border-l-4 border-purple-700 pl-4 py-2 max-w-4xl',
							...
							'warning' => [
								'border-l-4 border-yellow-600 pl-4 py-2 max-w-4xl',
							...
							'caution' => [
								'border-l-4 border-red-500 pl-4 py-2 max-w-4xl',
